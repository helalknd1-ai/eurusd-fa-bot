        ...
        resp = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "قوانین:\n"
                    "- خبر صعودی یورو (مثلاً دلار ضعیف، کاهش نرخ، تورم پایین آمریکا) = bull\n"
                    "- خبر نزولی یورو (مثلاً دلار قوی، افزایش نرخ، تورم بالا آمریکا) = bear\n"
...
def summarize_speech_ai(speech_texts, speaker_name=""):
    ...
    try:
        ...
        resp = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "تو تحلیل‌گر اقتصادی هستی. متن سخنرانی یا اظهارات مقامات پولی را "
                    "به فارسی روان و کوتاه خلاصه می‌کنی.\n\n"
                    "خروجی به این شکل:\n"
                    "۱) سه تا پنج نکته کلیدی (هر کدام یک خط)\n"
                    "۲) تأثیر بر یورو/دلار (صعودی/نزولی/خنثی) با دلیل\n\n"
